 ...
 */
function getEnabledBreakpoints() {
 ...
 */
async function getPausedStack(session: vscode.DebugSession) {
 ...
 */
async function getTopFrameVariables(session: vscode.DebugSession) {
  ...
  const results: { scopeName: string; variables: { name: string; value: string }[] }[] = [];
...
export interface PausedState {
  breakpoints: ReturnType<typeof getEnabledBreakpoints>
  pausedStack: Awaited<ReturnType<typeof getPausedStack>>
  topFrameVariables: Awaited<ReturnType<typeof getTopFrameVariables>>
}

/**
 * Collects the current paused state: enabled breakpoints, call stack, and top-frame local variables.
 */
export async function gatherPausedState(session: vscode.DebugSession): Promise<PausedState> {
  if (!session) {
  ...
  try {
    result.breakpoints = getEnabledBreakpoints();
  } catch (error) {
    log.error("Error getting enabled breakpoints", String(error));
  }
  try {
    result.pausedStack = await getPausedStack(session);
  } catch (error) {
    log.error("Error getting paused stack", String(error));
  }
  try {
    result.topFrameVariables = await getTopFrameVariables(session);
  } catch (error) {
